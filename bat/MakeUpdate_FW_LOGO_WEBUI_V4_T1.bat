...
@if not exist %FILE_NAME% goto usage5
set FILE_NAME=11-005b0000-WEBUI.bin
@if not exist %FILE_NAME% goto usage5
...
@ren *WEBUI*.bin 11-005b0000-WEBUI.bin
...
@echo on
sign.exe ./xml/packet_fw_logo_webui_V4_T1.xml -p %1 %2 %4
...
C:\Python25\python compress.py c ./Images/11-005b0000-WEBUI.bin ./%ZLIB_FILE_TEMP%/11-005b0000-WEBUI.zlib 9

@REM delete all mbn, bin files in the current directory
@if exist *.mbn del /f /a *.mbn
@if exist *.bin del /f /a *.bin

merge.exe ./xml/packet_fw_logo_webui_V4_T1.xml -p :%1:%2 %3_UPDATE_%2.BIN
merge.exe ./xml/packet_fw_logo_webui_V4_zip_T1.xml -p :%1:%2 %3_UPDATE_%2.ZIP
